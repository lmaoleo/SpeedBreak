...
namespace BNG {
    ...
    public class SmoothLocomotion : MonoBehaviour {
        ...
        public delegate Vector3 ModifyMovementAction(Vector3 motion);
        public event ModifyMovementAction OnProcessMovement;
        #endregion

        Vector3 ApplyMovementProcessors(Vector3 motion) {
            if (OnProcessMovement == null) {
                return motion;
            }

            var processors = OnProcessMovement.GetInvocationList();
            for (int i = 0; i < processors.Length; i++) {
                motion = ((ModifyMovementAction)processors[i])(motion);
            }
        ...
        public virtual void MoveCharacter() {
            ...
            moveDirection = ApplyMovementProcessors(moveDirection);
